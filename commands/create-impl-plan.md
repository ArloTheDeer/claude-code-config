...
4.  **生成任務與任務說明：** 基於 PRD 分析和現狀評估，創建檔案並生成實作功能所需的主要任務。根據判斷決定使用多少個任務，通常約為 5-7 個。為每個任務生成詳細的任務說明（以條列式呈現），用於與開發者溝通開發範圍和實作細節，確保涵蓋 PRD 暗示的所有實作要點。**若使用者在對話中提及「TDD」，則建議標註「使用 TDD 開發流程」。** 同時為每個任務生成「完成檢查」，提供 1-5 條簡要的檢查要點，說明如何驗證該任務已完成（詳見「完成檢查生成指引」章節）。
...
**相關檔案**
- `path/to/file.ts` - 主要實作檔案
- `path/to/file.test.ts` - 單元測試檔案
...
若使用者在對話中提及「TDD」，則在程式開發相關任務的標題後附註「使用 TDD 開發流程」。

**注意**：只有涉及程式邏輯實作的任務需要 TDD 標註。環境設定、文件撰寫、驗收測試等任務不需要標註。

<example>
```markdown
## 任務概要
- [ ] 實作用戶註冊功能 - 使用 TDD 開發流程
- [ ] 執行驗收測試
- [ ] 更新專案文件
